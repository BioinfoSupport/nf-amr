...
select_filter <- function(element_id) {
...
contigs |>
	select(assembly_id,contig_id,contig_length,GC,topology,is_plasmid) |>
	mutate(GC = 100*GC) |>
	reactable(
		pagination = FALSE,
		compact = TRUE,
	  ...
	  defaultColDef = colDef(headerClass = "header", align = "left"),
		...
		columns = list(
	      ...
	      style = function(value) {
	        bar_style(width = value / max(contigs$contig_length), fill = "#DDDDFF", color = "#000000")
	      },
	      ...
	      format = colFormat(digits = 1,suffix = "%")
			...
			topology = colDef(
				filterInput = select_filter('contigs-select')
			),
			is_plasmid = colDef(
				filterInput = select_filter('contigs-select')
			)
...
### amrfinder+
This can be a nice representation for a single sample
```{r}
amrfinderplus <- db |>
